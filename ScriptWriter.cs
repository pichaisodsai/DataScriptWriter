...
namespace DataScriptWriter
{
    public class ScriptWriter
    {
        ...
        private const int MaxRowsPerFile = 100000;
        ...
        public void GenerateForTable(ScriptObject so)
        {
            ...
            int totalFiles = (mainTable.Rows.Count + MaxRowsPerFile - 1) / MaxRowsPerFile;

            for (int fileIndex = 0; fileIndex < totalFiles; fileIndex++)
            {
                string filename = totalFiles == 1 ? Path.Combine(_OutputFolder, $"{so.FullName}.sql") : Path.Combine(_OutputFolder, $"{so.FullName}_{fileIndex + 1}.sql");
                using (System.IO.StreamWriter w = new System.IO.StreamWriter(filename))
                {
                    ...
                    if (OptionProcWrapUp)
                    {
                        ...
                        {
                            w.WriteLine(String.Format("CREATE PROCEDURE [{0}].[Populate_{1}_{2}]", "dbo", so.TempCounterpart.Replace("#", ""), fileIndex + 1));
                        }
                        w.WriteLine("AS");
                        w.WriteLine("BEGIN");
                    }
                ...
                Console.WriteLine($"{so.FullQuoted} was scripted (part {fileIndex + 1}).");
            }
        ...
        private void ScriptTableMerge(ScriptObject so, DataTable colInfoTable, DataTable mainTable, System.IO.StreamWriter w, string colList, string scolList, string PKcolListOn, string colCastList, int fileIndex, int totalFiles)
        {
            ...
            // Change the foreach loop to a while loop and add a condition for rowIndex
            while (rowIndex < mainTable.Rows.Count && rowIndex < (fileIndex + 1) * MaxRowsPerFile)
            {
                DataRow row = mainTable.Rows[rowIndex];
                //Begin Insert to temp
                if (rowIndex % MaxRowsPerBatch == 0 || rowIndex % MaxRowsPerFile == 0)
                {
                ...
                //End Insert to Temp
                if (rowIndex % MaxRowsPerBatch == 0 || rowIndex == mainTable.Rows.Count || rowIndex == (fileIndex + 1) * MaxRowsPerFile)
                {
